...
I had this code for many years and few days ago decided to share it with you.

It's not written by me, but still worth giving it a try if you want to have some inspiration and improve it the way you want to.

Compile with `gcc -o player player.c -Wall -Wextra -O2 \`pkg-config --libs --cflags libvc gtk+-3.0\``

You can give it a try and improve this code by adding volume slider button.
...
void 
player_widget_on_realize (GtkWidget *widget, gpointer data) {
...
void 
on_playpause (GtkWidget *widget, gpointer data) {
...
void 
on_stop (GtkWidget *widget, gpointer data) {
    pause_player ();
...
play (void) {
    ...
    pause_icon_image = gtk_image_new_from_icon_name ("media-playback-pause", 
...
pause_player (void) {
    ...
    play_icon_image = gtk_image_new_from_icon_name ("media-playback-start", 
    											   GTK_ICON_SIZE_BUTTON);
